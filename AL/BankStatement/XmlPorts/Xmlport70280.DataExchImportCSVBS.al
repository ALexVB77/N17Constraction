xmlport 70280 "Data Exch. Import - CSV BS"
{
    Caption = 'Data Exch. Import - CSV BS';
    Direction = Import;
    Format = VariableText;
    ...
    begin
        ...
        if savedColumnValue <> '' then begin
            IF (DataExchField.GET(DataExchEntryNo, ImportedLineNo, columnNumber)) THEN BEGIN
                DataExchField.VALIDATE(Value, COPYSTR(DataExchField.Value + savedColumnValue, 1, MAXSTRLEN(DataExchField.Value)));
                DataExchField.MODIFY(TRUE);
            END
            ELSE BEGIN
                DataExchField.Init();
