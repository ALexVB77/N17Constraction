codeunit 70016 "FA Reclass. Transfer Line Ext"
{
    ...
    begin
        with FAReclassJnlLine do begin
            ...
            PostToGenJnL := ("FA No." <> "New FA No.") or
                ("Depreciation Book Code" <> "New Depreciation Book Code") or
                ("FA Posting Group" <> "New FA Posting Group");
            ...
            if ("FA No." = "New FA No.") and ("Depreciation Book Code" = "New Depreciation Book Code") and
               ("FA Posting Group" = "New FA Posting Group") and
                ...
                for OldNewFA := 0 to 1 do begin
                    ...
                    if TransferType[i] and (Amounts[i] <> 0) then begin
                        ...
                        if TransferToGenJnl then
                            InsertGenJnlLine(FAReclassJnlLine, CurrFA, Sign * Amounts[i], "Insert Bal. Account", OldNewFA = 1)
                        else
                            InsertFAJnlLine(FAReclassJnlLine, CurrFA, Sign * Amounts[i], OldNewFA = 1);
                        Done := true;
                    end;
                end;
            end;
                ...
                if TransferType[1] and (Amounts[1] <> 0) then begin
                    ...
                    if TransferToGenJnl then begin
                        ...
                        if TransferAmntToGenJnl then
                            InsertGenJnlLine(FAReclassJnlLine, NewFA, Sign * Amounts[1], true, OldNewFA = 1)
                        else
                            InsertFAJnlLine(FAReclassJnlLine, NewFA, Amounts[1], OldNewFA = 1);
                    end;
                end;
        end;
    end;
    ...
    local procedure InsertFAJnlLine(var FAReclassJnlLine: Record "FA Reclass. Journal Line"; FA: Record "Fixed Asset"; EntryAmount: Decimal; NewLine: Boolean)
    begin
        ...
        with FAJnlLine do begin
            ...
            if NewLine then
                "FA Posting Group" := FAReclassJnlLine."New FA Posting Group";
    ...
    local procedure InsertGenJnlLine(var FAReclassJnlLine: Record "FA Reclass. Journal Line"; FA: Record "Fixed Asset"; EntryAmount: Decimal; BalAccount: Boolean; NewLine: Boolean)
    var
    ...
    begin
        ...
        if NewLine then
            GenJnlLine."Posting Group" := FAReclassJnlLine."New FA Posting Group"
        else
